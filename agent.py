...
def manhattan_heuristic(r,c,d):
    return (ROWS - r) + (COLS - c) + d
...
class DFSSearchAgent(SearchAgent):
    ...
    def dfs_action(self, row, col, observations):
        r = observations[0]
        c = observations[1]
    ...
    def start(self, time, observations):
        ...
        return self.dfs_action(row, col, observations)
...
class IdaStarSearchAgent (SearchAgent):
    ...
    def __init__(self):
        ...
        self.backpointers = {}
        self.reset()
        self.depth = 0 #the depth of the current node
        self.limit = ROWS + COLS #heuristic limit
    ...
    def dfs_action(self, r, c, observations):
        """
        Given:
        - some places we could go in our queue
        - some backtracking links on how we got there
        Return:
        - next step for agent to visit
        """
        if not self.goal:
            (r2, c2) = self.deque()
            self.mark_target(r2, c2)
        ...
        r2, c2 = self.goal
        #teleporting
##        if not (r == r2 and math.fabs(c - c2) == 1) or (c == c2 and math.fabs(r - r2) == 1):
##            get_environment().teleport(self, r2, c2)
##            
##        else:
        dr, dc = r2 - r, c2 - c
        ...
        v = self.constraints.get_instance() #make the action vector to return
        if action is not None and observations[2 + action] == 0:
            v[0] = action  #do that action if no obstacles
        else:
            (r2,c2) = self.get_next_step(r,c,r2,c2)
            dr, dc = r2 - r, c2 - c
            v[0] = get_action_index((dr,dc)) #what action
        return v # return the action

    def visit(self, r, c, observations):
        
        #visits the node and then decide where to go next
        # DEBUG print "depth: ", self.depth, "\n"
        # DEBUG print "depthdictionary upon visit: ", self.depthdictionary, "\n"

        self.depth = self.depthdictionary[(r,c)] + 1  
        if self.heuristic(r,c,self.depthdictionary[(r,c)]) > self.limit:
            print "limit before reset is ", self.limit
            self.limit += 1
            self.reset()
            get_environment().cleanup()
            get_environment().teleport(self,0,0)
            print "limit after reset is ", self.limit, "\n\n\n"
            
        else:
            # if we have not been here before, build a list of other places we can go
            if (r,c) not in self.visited:
                self.mark_visited(r, c)
            self.visited.add((r, c))
            self.enqueued.add((r, c))
            if (self.goal == (r, c)):
                print 'reached goal: ' + str((r, c))
                self.goal = None

            #queue places to go next
            for m, (dr,dc) in enumerate(MAZE_MOVES):    
                if not observations[2 + m]: # can we go that way?
                    r2 = r + dr  #row we could move to
                    c2 = c + dc  #col we could move to
                    if (r2,c2) not in self.enqueued:
                        self.mark_the_front(r,c,r2,c2)
                        self.enqueued.add((r2,c2))
                        self.depthdictionary[(r2,c2)] = self.depth
                        self.enque((r2,c2))
                        assert self.backpointers.get((r,c)) != (r2,c2)
                        self.backpointers[(r2,c2)] = r, c #remember where we would come from
            #DEBUG print "depthdictionary after visiting: ", self.depthdictionary, "\n"
    ...
    def reset(self):
        ...
        self.enqueued = set ([])
        self.goal = None
        self.heuristic = manhattan_heuristic # distance to goal + distance traveled
        self.depthdictionary = {(0,0):0}
        get_environment().cleanup() 
    
        
    def enque(self, cell):
        (r,c) = cell
        tempdepth = self.depthdictionary[(r,c)]
        heappush(self.queue, Cell(self.heuristic(r,c,tempdepth), r, c))
